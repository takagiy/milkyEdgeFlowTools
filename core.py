...
def _pava(values):
    ...
    for x in values:
        ...
        while len(pooled) > 1 and pooled[-2] > pooled[-1]:
            value = pooled.pop()
            count = counts.pop()
            pooled[-1] = ((pooled[-1] * counts[-1] + value * count)
                          / (counts[-1] + count))
            counts[-1] += count
    out = []
    for value, count in zip(pooled, counts):
        out.extend([value] * count)
    return out


def opposite_shore_params(points, curve, clamp_ends=True, ratios=None):
    ...
    Closest-point projection saturates where the source chain extends past
    the rail (several points hit the rail endpoint) and can locally invert
    on curvy chains. Inversions are pooled monotonically (PAVA) and every
    flat run is then redistributed between its reliable neighbors using
    the source chain's arc `ratios` (index-uniform when omitted), so the
    result stays strictly increasing without cascading compression.
    """
    length = curve.total_length
    ...
    if n == 0:
        return []
    if ratios is None:
        ratios = [i / max(1, n - 1) for i in range(n)]
    if clamp_ends:
        params[0] = 0.0
        if n > 1:
            params[-1] = length
    ...
    params = _pava(params)
    tolerance = 1.0e-6 * max(length, 1.0e-9)
    i = 0
    while i < n:
        j = i
        while j + 1 < n and params[j + 1] - params[i] <= tolerance:
            j += 1
        if j > i:
            lo, hi = i, j
            if i == 0:
                lo = i + 1        # the head keeps its projected value
            if j == n - 1:
                hi = j - 1        # the tail keeps its projected value
            left_i = lo - 1
            right_i = hi + 1
            if lo <= hi and right_i < n:
                r_span = ratios[right_i] - ratios[left_i]
                v_left = params[left_i]
                v_right = params[right_i]
                if r_span > 1.0e-12:
                    for k in range(lo, hi + 1):
                        t = (ratios[k] - ratios[left_i]) / r_span
                        params[k] = v_left + t * (v_right - v_left)
        i = j + 1
